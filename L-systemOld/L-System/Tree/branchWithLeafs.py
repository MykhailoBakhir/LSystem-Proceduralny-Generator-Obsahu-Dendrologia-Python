...
def interpret_lsystem_Branch(lsys_string, start, direction, length, initial_radius, taper, random_pi, angle_deg=60, ):
    ...
    for symbol in lsys_string:
        if symbol == 'F':
            ...
            bottom_radius = current_radius * 1.04
            ...
            random_angle = (angle_digit / 9.0) * 10  
            ...
            if (int)(random_pi[pi_index]) > 7:
                ...
                leaf_lsystem = leafForTree.expand_lsystem(leaf_axiom, leaf_rules, iterations=6)
                leaf_mesh = leafForTree.create_leaf_geometry(leaf_lsystem, angle_deg=16, step_length=length/8)
