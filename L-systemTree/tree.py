...
def expand_lsystem(axiom, rules, iterations, seed, branch_deletion_chance):
    pi_index = seed
    result = axiom
    for _ in range(iterations):
        ...
        for symbol in result:
            if symbol in {'B', 'C', 'D', 'E'}:
                ...
                pi_index += 1
                digit_prob = digit / 9
                if digit_prob >= branch_deletion_chance:
...
def create_centered_tilted_frustum(start, direction, length, bottom_radius, top_radius, tilt_angle, sections, tilt_axis=[0,0,1]):
...
def interpret_lsystem(lsys_string, start, direction, length, initial_radius, taper, angle_variation, angle_deg, sections):
    meshes = []
    ...
    while i < len(lsys_string):
        ...
        if symbol == 'F':
            ...
            for _ in range(count):
                ...
                angle_now = (angle_digit / 9.0) * angle_variation - angle_variation / 2
                angle_next = (future_angle_digit / 9.0) * angle_variation - angle_variation / 2
...
def generate_tree(seed, length, initial_radius, taper, sections, iterations, angle_deg, angle_variation, branch_deletion_chance):
    ...
    lsystem = expand_lsystem(axiom, rules, iterations=iterations, seed=seed, branch_deletion_chance=branch_deletion_chance )
    print(lsystem)
