...
def interpret_lsystem(lsys_string, start, direction, length, initial_radius, taper, angle_deg=15):
    ...
    for symbol in lsys_string:
        if symbol == 'F':
            ...
            current_dir = rotate_direction(current_dir, +angle_deg, axis=[1, 0, 0])  # X
        elif symbol == '-':
            current_dir = rotate_direction(current_dir, -angle_deg, axis=[1, 0, 0])  # X
        elif symbol == '&':
            current_dir = rotate_direction(current_dir, +angle_deg, axis=[0, 1, 0])  # Y
        elif symbol == '^':
            current_dir = rotate_direction(current_dir, -angle_deg, axis=[0, 1, 0])  # Y
        elif symbol == '\\':
            current_dir = rotate_direction(current_dir, +angle_deg, axis=[0, 0, 1])  # Z
        elif symbol == '/':
            current_dir = rotate_direction(current_dir, -angle_deg, axis=[0, 0, 1])  # Z
        elif symbol == '|':
            current_dir = rotate_direction(current_dir, 180, axis=[1, 0, 0])         # X 180°
        elif symbol == '[':
...
lsystem = "FFF[++FFF][&&FF[^^F]F]FF[--FF]F[&&FF]F"
tree = interpret_lsystem(lsystem, start=[0, 0, 0], direction=[0, 0, 1], length=5, initial_radius=4.0, taper=0.8)
tree.export("lsystem_rotated.obj")
